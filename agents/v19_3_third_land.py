...
def agent(observation: Any, configuration: Any = None):
    ...
    # Run the proven control exactly once, then post-process only the market list.
    ...
    # The old architecture can never reach Q3 because its expansion predicate is
    # len(unlocked)==1.  Here the third district is a planned capital step: buy it
    # only after Q2 is alive and cash remains above a conservative operating floor.
    q3_eligible = (
        len(unlocked) == 2
    ...
    if q3_eligible and not any(str(o[0]).upper() == "BUY_LAND" for o in market):
        # Preserve feed survival orders ahead of capex; insert before discretionary
        # sells/seed buys/hiring.  The environment's 10-order cap is respected.
        idx = 0
